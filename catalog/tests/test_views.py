...
class LoanedBookInstancesByUserListViewTest(TestCase):
    def setUp(self):
        ...
        for book_copy in range(number_of_book_copies):
            # the last part is a way to randomize the numbers, nothing special
            return_date = timezone.localtime() + datetime.timedelta(days=book_copy%5)
            # this is also a way to randomize the owner
...
class AuthorCreateTest(TestCase):
    ...
    def SetUp(self):
        standard_user = User.objects.create_user(username='standard_user', password='1X<ISRUkw+tuK')
        user_with_permission = User.objects.create_user(username='user_with_permission', password='2HJ1vRV0Z&3iD')
    ...
    def test_access_for_url(self):
        response = self.client.get('/catalog/author/create')
        self.assertNotEqual(response.status_code, 404)

    def test_access_for_url_by_name(self):
        response = self.client.get(reverse('author-create'))
        self.assertNotEqual(response.status_code, 404)    

    def test_access_for_not_logged_in_user(self):
        response = self.client.get(reverse('author-create'))
        self.assertRedirects(response, '/accounts/login/?next=/catalog/author/create/')

    def test_access_for_user_without_permission(self):
        self.client.login(username='standard_user', password='1X<ISRUkw+tuK')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 403)

    def test_access_for_user_with_correct_permission(self):
        self.client.login(username='user_with_permission', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 200) 